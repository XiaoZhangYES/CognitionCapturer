...
class FeatureGet:
    ...
    def _get_model(self):
        vlmodel, preprocess_train, _ = utils.create_model_and_transforms(
            'ViT-H-14',
            pretrained='/HDD2/Things_dataset/model_pretrained/clip/open_clip_pytorch_model.bin',
            output_dict=True,
            precision='fp32',
            device=self.device)
        # todo 得到局部特征？
    ...
    def _Textencoder(self, text):
        ...
        with torch.no_grad():
            text_feature = self.vlmodel._encode_text(text_input)
        ...
        with torch.no_grad():
            ...
            for text_file_name, img_file_name in zip(['texts_BLIP2_test.npy'], ['test_image.npy']):
                ...
                torch.save(text_features_dict,
                           f"/HDD2/Things_dataset/model_pretrained/data_features/text_{type}_features_clip_dict_test.pt")
        ...
        with torch.no_grad():
            ...
            for i, file_name in enumerate(['test_image.npy']):
                ...
                torch.save(image_features_dict,
                           f"/HDD2/Things_dataset/model_pretrained/data_features/image_{type}_features_clip_dict_test.pt")
...
if __name__ == "__main__":
    instance = FeatureGet()
    instance.get_feature("depth")
    instance.get_feature("original")
    instance.get_feature("aug")
